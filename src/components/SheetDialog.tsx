...
interface Props {
  /** Null while there is nothing to show. Any truthy value opens the dialog. */
  data: unknown;
  /** Draws the sheet. Called once the brand faces have loaded. */
...
/**
 * The preview that is also the file.
 *
 * Shared by the quote and the board because the awkward half is the same for
 * both: waiting on the fonts before drawing, and saving through the host when
 * the page is inside an embedded viewer and cannot save on its own.
 */
export default function SheetDialog({ data, render, filename, title, onClose }: Props) {
